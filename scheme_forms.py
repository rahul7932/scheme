...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        expression = scheme_eval(expressions.rest.first,env)
        env.define(signature, expression)
        return signature 
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        expression = scheme_eval(bindings.first.rest.first,env)
        names = Pair(bindings.first.first,names)
        values = Pair(expression,values)
